...
class InstantonWithGravity(SingleFieldInstanton):
    """
    This class will calculate properties of an instanton with a single scalar
    field including gravity using the overshoot/undershoot method. 

    The input parameters are the same as for :class:`SingleFieldInstanton`,
    except that `alpha` now defaults to 3 and there is a new `M_Pl` parameter
    which should be set equal to the Planck in whichever units are used to
    describe the field. Note that rescaling the potential (for fixed field
    values) still just results in an overall rescaling of the size of the
    instanton, whereas rescaling the field values relative to `M_Pl` will now
    result in a different instanton shape.
    """
    def __init__(self, phi_absMin, phi_metaMin, V, 
                 dV=None, d2V=None, phi_eps=1e-3, alpha=3,
                 phi_bar=None, rscale=None, M_Pl=1.0):
        self.kappa = 8*np.pi/(3*M_Pl**2)
        SingleFieldInstanton.__init__(
            self, phi_absMin, phi_metaMin, V, 
            dV=None, d2V=None, phi_eps=1e-3, alpha=3,
            phi_bar=None, rscale=None)
    ...
    def initialConditions(self, delta_phi0, rmin, delta_phi_cutoff):
        ...
        for the angular radius `rho(r0)`. However, the coupled set of
        differential equations for `rho` and `phi` is non-linear, so we cannot
        plausibly find an analytic solution. Instead, this function assumes
        that `dphi/dr` is small and `V(phi)` is approximately constant for the
        purpose of calculating `rho(r)`, which can then be found analytically.
        An initial guess `r0_guess` is calculated using 
        :method:`SingleFieldInstanton.initialConditions`. A new initial radius
        is then chosen such that `(1/rho(r0))(drho/dr) = 1/r0_guess`. This way,
        the value of the friction term at `r0` is the same as it would be
        without gravity.

        Returns
        -------
        r0 : float
            Initial radius at which to start integration
        phi, dphi : float
            Field value and its derivative at r0
        rho, drho : float
            Angular radius and its derivative as a function of r at r0
        """
        r0_guess, phi0, dphi0 = SingleFieldInstanton.initialConditions(
            self, delta_phi0, rmin, delta_phi_cutoff)
        print "r0_guess, phi0, dphi0 {}, {}, {}".format(r0_guess, phi0, dphi0) 
        V0 = self.V(phi0)
        w = abs(self.kappa*V0)**0.5
        if w == 0:
            # No curvature.
            r0, rho0, drho0 = r0_guess, r0_guess, 1.0
        elif V0 > 0:
            # positive curvature
            r0 = np.arctan(w*r0_guess)/w
            rho0 = np.sin(w*r0)/w
            drho0 = np.cos(w*r0)
        else:
            # negative curvature
            if w*r0_guess >= 1:
                raise PotentialError(
                    ...
                    "stable, not metastable")
            r0 = np.arctanh(w*r0_guess)/w
            rho0 = np.sinh(w*r0)/w
            drho0 = np.cosh(w*r0)
        return self._initialConditionsWithGrav_rval(
            r0, phi0, dphi0, rho0, drho0)
    ...
    def equationOfMotion(self, y, r):
        ...
        return np.array([
            dphi,
            self.dV(phi) - self.alpha*drho*dphi/rho,
            drho,
            -self.kappa*rho*(dphi*dphi + self.V(phi))
        ])

    _integrateProfileWithGrav_rval = namedtuple(
        "integrateProfile_rval", "r y convergence_type")
    def integrateProfile(self, r0, y0, dr0, 
                         epsfrac, epsabs, drmin, rmax, *eqn_args):
        R"""
        ...
        .. math::
          \frac{d^2\phi}{dr^2} 
            ...
            \frac{dV}{d\phi} \\
          \frac{d^2\rho}{dr^2} = 
            -\frac{8\pi}{3M_{Pl}^2}\left[\left(\frac{d\phi}{dr}\right)\right]^2
            + V(\phi)\right]

        This works the same basic way as when there is no gravity, 
        except now the overshoot/undershoot conditions are a little bit more
        complicated. The solution will overshoot if at any point `rho(r) = 0`
            ...
            if r1 > rmax:
                ...
                if y1[3] < 0 and (y1 < 25*epsabs)[2]:
                    ...
                    if (abs(y) < 3*epsabs)[1]: 
                        ...
                        convergence_type = "overshoot"
                raise IntegrationError("dr < drmin")
            elif (abs(y1[:2] - [self.phi_metaMin,0]) < 3*epsabs[:2]).all():
                r,y = r1,y1
                convergence_type = "converged"
                break
            elif y1[1]*ysign > 0 or (y1[0]-self.phi_metaMin)*ysign < 0:
    ...
    profile_rval = namedtuple("Profile1DWithGrav", 
        "R Phi dPhi Rho dRho Rerr")
    ...
    def findProfile(self, xguess=None, xtol=1e-4, phitol=1e-4, 
        ...
        epsfrac = np.array([1,1,1,1]) * phitol
        delta_phi_cutoff = thinCutoff * delta_phi
            # The sign for delta_phi_cutoff doesn't matter
        # --
        integration_args = (dr0, epsfrac, epsabs, drmin, rmax)
        ...
        while True:
            ...
            print "init params {}, {}, {}".format(delta_phi0, rmin, delta_phi_cutoff)
            ry0 = self.initialConditions(delta_phi0, rmin, delta_phi_cutoff)
            print "init out params {}".format(ry0)
        ...
        if max_interior_pts > 0:
            ...
            w = abs(self.kappa*V0)**0.5
            if w == 0:
                R_int2 = Rho_int = R_int
                dRho_int = np.ones_like(R_int)
            elif V0 > 0:
                R_int2 = np.tan(w*R_int)/w
                Rho_int = np.sin(w*R_int)/w
                dRho_int = np.cos(w*R_int)
            else:
                R_int2 = np.tanh(w*R_int)/w
                Rho_int = np.sinh(w*R_int)/w
                dRho_int = np.cosh(w*R_int)
            for i in xrange(1,len(R_int)):
                Phi_int[i], dPhi_int[i] = self.exactSolution(
                                               R_int2[i], Phi_int[0], dV, d2V)
            R = np.append(R_int, profile.R)
            Phi = np.append(Phi_int, profile.Phi)
            dPhi = np.append(dPhi_int, profile.dPhi)
            Rho = np.append(Rho_int, profile.Rho)
            dRho = np.append(dRho_int, profile.dRho)
