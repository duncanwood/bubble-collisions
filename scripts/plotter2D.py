...
plt.title("phi1 dot")
plt.legend()
plt.savefig("phi1dot_time_x.png")
plt.figure()
...
plt.title("phi2 dot")
plt.legend()
plt.savefig("phi2dot_time_x.png")
plt.figure()
...
plt.figure()
nx = 100
Xp = np.linspace(-0.4, 0.4,nx)[:,None] * np.ones((1,nx))
Y = np.linspace(0.0, 0.02,nx)[None,:] * np.ones((nx,1))
...
nx = 100
Xp = np.linspace(-0.4,0.4,nx)[:,None] * np.ones((1,nx))
Y = np.linspace(0.0,0.08,nx)[None,:] * np.ones((nx,1))
XY = np.rollaxis(np.array([Xp,Y]), 0, 3)
Z = V2D(Xp,Y)
plt.contour(Xp,Y,Z, np.linspace(np.min(Z), np.max(Z), 200), linewidths=0.5, zorder=0)
plt.colorbar()
...
nx = 100
Xp = np.linspace(-0.4,0.4,nx)[:,None] * np.ones((1,nx))
Y = np.linspace(0.0,.02,nx)[None,:] * np.ones((nx,1))
XY = np.rollaxis(np.array([Xp,Y]), 0, 3)
Z = V2D(Xp,Y)
plt.contour(Xp,Y,Z, np.linspace(np.min(Z), np.max(Z), 200), linewidths=0.5, zorder=0)
plt.colorbar()
